...
<template>
    <header class="flex items-center justify-between px-5 sm:px-10 py-[14px] sm:py-[18px] border-b border-[#E7E5E0] sticky top-0 bg-[#F7F5F1] z-[100]">
        <!-- will be replaced with actual logo -->
        <div class="font-serif text-[15px] sm:text-[17px] font-medium text-[#1C1917] tracking-tight shrink-0 mr-3">Adullam Academy</div>

        <div class="relative" id="switcher-wrap">
            <!-- Trigger pill — always shows parent account -->
            <button
                class="switcher-trigger flex items-center gap-[8px] sm:gap-[10px] py-[6px] pr-[10px] sm:pr-[14px] pl-[8px] border border-[#E7E5E0] rounded-[40px] cursor-pointer transition-[border-color] duration-[180ms] bg-[#F7F5F1] font-sans text-base hover:border-[#B45309]"
            ...
            >
                ...
                <span class="text-[14px] font-medium max-[480px]:hidden">{{ parent.name }}</span>
                <span class="switcher-caret text-[10px] text-[#78716C] ml-[2px] transition-transform duration-200 inline-block" :class="{ 'rotate-180': showDropDown }" aria-hidden="true">▾</span>
            </button>

            <!-- Dropdown panel -->
            <div
                v-show="showDropDown"
                class="switcher-dropdown absolute top-[calc(100%+8px)] right-0 w-[calc(100vw-40px)] max-w-[280px] min-w-[228px] bg-[#F7F5F1] border border-[#E7E5E0] rounded-[10px] shadow-[0_4px_20px_rgba(28,25,23,0.09),0_1px_3px_rgba(28,25,23,0.05)] overflow-hidden z-[200]"
            ...
            >
                ...
                <!-- Children list -->
                <div class="text-[10px] font-medium tracking-[0.08em] uppercase text-[#78716C] px-4 pt-[10px] pb-[5px]">Viewing dashboard for</div>
                ...
                <!-- Log out -->
                <button
                    class="dd-logout-btn flex items-center gap-[9px] w-full pt-[10px] px-4 pb-[14px] bg-transparent border-none cursor-pointer font-sans text-[13px] text-[#78716C] transition-[color] duration-[120ms] text-left hover:text-[#1C1917]"
